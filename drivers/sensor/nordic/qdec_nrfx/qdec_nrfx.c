...
static int qdec_nrfx_init(const struct device *dev)
{
	...
	nerr = nrfx_qdec_init(&dev_data->qdec, &config->config, qdec_nrfx_event_handler, (void *)dev);
	if (nerr != 0) {
		return -EALREADY;
	}
...
#define QDEC_PM_ISR_SAFE(inst)										\
	COND_CODE_1(											\
		UTIL_AND(										\
			...
			UTIL_AND(									\
				DT_NODE_HAS_PROP(DT_DRV_INST(inst), power_domains),			\
				DT_NODE_HAS_STATUS_OKAY(DT_PHANDLE(DT_DRV_INST(inst), power_domains))	\
			)										\
		),											\
		(0),											\
		(PM_DEVICE_ISR_SAFE)									\
...
#define SENSOR_NRFX_QDEC_DEVICE(inst)								     \
	...
	static struct qdec_nrfx_config qdec_##inst##_config = {					     \
		...
		.enable_pin = DT_PROP_OR(DT_DRV_INST(inst), enable_pin, NRF_QDEC_PIN_NOT_CONNECTED), \
